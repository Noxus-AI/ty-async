...
const STDLIB_BLOCKING: &[(&str, &str)] = &[
    ...
    // filesystem — single-syscall metadata ops (os.stat, os.listdir, os.mkdir,
    // os.remove, os.rename, ... and all of os.path) are deliberately absent:
    // they're cheap enough that flagging them is mostly noise
